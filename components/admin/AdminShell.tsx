...
function AdminSidebar({ mobile = false }: { mobile?: boolean }) {
  ...
  return (
    <aside className={`${mobile ? "w-full relative min-h-0 flex-1" : "hidden lg:flex w-64 fixed inset-y-0 left-0 z-20"} flex flex-col`}
      style={{ background: "rgba(4,11,6,0.85)", backdropFilter: "blur(24px) saturate(1.6)", borderRight: "1px solid rgba(255,255,255,0.06)" }}>
      {/* Logo */}
...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const close = () => { if (wide.matches) setMenuPath(null); };
    wide.addEventListener("change", close);
    return () => { wide.removeEventListener("change", close); node?.close(); };
  }, [open]);
  if (pathname === "/admin/giris") return <>{children}</>;
  return (
    <AdminProvider>
      <div className="relative min-h-screen">
        <div className="nature-bg"><div className="nature-orb nature-orb-1" /><div className="nature-orb nature-orb-2" /></div>
        <div className="relative z-10 min-h-screen">
          <div className="lg:hidden sticky top-0 z-20 bg-[#040b06] p-3 border-b border-white/10">
            <button type="button" aria-expanded={open} aria-controls="admin-menu" onClick={() => setMenuPath(pathname)} className="min-h-11 px-4 text-white rounded-xl border border-white/20">Yönetim menüsü</button>
          </div>
          <AdminSidebar />
          <dialog onKeyDown={containDialogTab} id="admin-menu" ref={dialog} onCancel={() => setMenuPath(null)} aria-label="Yönetim menüsü" className="m-0 h-dvh max-h-none w-80 max-w-[90vw] bg-[#040b06] text-white p-0 backdrop:bg-black/60">
            <div className="flex h-full flex-col" onClick={(e) => { if ((e.target as HTMLElement).closest("a")) setMenuPath(null); }}>
              <button type="button" className="min-h-11 self-end px-5" onClick={() => setMenuPath(null)}>Kapat ×</button>
              <AdminSidebar mobile />
            </div>
          </dialog>
